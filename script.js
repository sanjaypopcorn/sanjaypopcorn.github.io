...
function calculate() {
    try {
        ...
        } else if (aircraft.type === "p2006t") {
            handleP2006T(aircraft);
        } else {
...
function handleP2006T(aircraft) {
    ...
    const aircraftMoment = aircraft.weight*aircraft.arm;
    const fuelMoment = fuelLiters*0.72*0.755
    const pilotMoment = pilotWeight*0.893;
    const copilotMoment = copilotWeight*0.893;
    ...
    // Update the results display
    document.getElementById("aircraftResult").innerHTML = `<td>Aircraft</td><td>${aircraft.weight.toFixed(2)}</td><td>${aircraft.arm}</td><td>${aircraftMoment.toFixed(4)}</td>`;
    document.getElementById("pilotResult").innerHTML = `<td>Pilot</td><td>${pilotWeight.toFixed(2)}</td><td>${-0.893}</td><td>${-pilotMoment.toFixed(2)}</td>`;
    document.getElementById("copilotResult").innerHTML = `<td>Copilot</td><td>${copilotWeight.toFixed(2)}</td><td>${-0.893}</td><td>${-copilotMoment.toFixed(2)}</td>`;
    document.getElementById("fuelResult").innerHTML = `<td>Fuel</td><td>${fuelWeight.toFixed(2)}</td><td>${0.755}</td><td>${fuelMoment.toFixed(4)}</td>`;
...
function handleP2010(aircraft) {
    ...
    const baggageInput = document.getElementById("baggage-weight").value;
    const baggageWeight = baggageInput !== "" ? parseFloat(baggageInput) : 30;
    ...
    const baggageMoment = baggageWeight * 1.612;

    const takeoffWeight = dryWeight + tofuelWeight + baggageWeight;
    const landingWeight = dryWeight + ldgfuelWeight + baggageWeight;
...
document.getElementById('aircraft-select').addEventListener('change', function() {
    ...
    const p2010BaggageInputs = document.getElementById('p2010-baggage-input');

    if (aircraft.type === 'p2010') {
        // Show the fuel inputs for P2010 aircraft
        p2010FuelInputs.style.display = 'block';
        p2010BaggageInputs.style.display = 'block';
    } else {
...
function old_handleP2010(aircraft) {
